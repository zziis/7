  ...
  renderMessages() {
    ...
    container.innerHTML = msgs.map(m => `
      <div class="flex items-start gap-3 ${m.isMe ? 'flex-row-reverse' : ''} animate-fadeIn">
        <div class="w-10 h-10 rounded-xl bg-slate-800 border border-cyan-500/30 flex items-center justify-center text-lg flex-shrink-0 shadow-sm overflow-hidden">
          ${/^https?:\/\//i.test(String(m.avatar||'')) ? `<img src="${m.avatar}" alt="${m.user}" class="w-full h-full object-cover">` : m.avatar}
        </div>
        <div class="max-w-[75%]">
