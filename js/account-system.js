class ZilzalAccountSystem {
  ...
  init(){
    ...
    // Show the login / guest chooser immediately.
    // restore() will hide it only after confirming a valid registered session.
    this.showGate();
    this.restore();
  }
  ...
  showGate(){
    this.closeDrawer();
    document.getElementById('zilzal-auth-gate')?.classList.remove('hidden');
  }
  ...
  async loadProfile(){
    const {data,error}=await this.sb.from('profiles').select('*').maybeSingle();
    if(error||!data){this.profile=null;this.guest=false;this.paint();return this.showGate();}
    this.profile=data;
    this.guest=false;
    this.paint();
    this.startTimer();
  ...
  needAccount(text='هذه الميزة للحسابات المسجلة فقط.'){
    ...
    document.getElementById('go-auth-from-modal')?.addEventListener('click',()=>{this.closeModal();this.showGate();});
  }
