class ZilzalAccountSystem {
  ...
  init(){
    ...
    this.bindUI();
    // v8.4: بوابة الدخول تظهر دائماً أول فتح الصفحة، ولا ندخل كزائر تلقائياً.
    this.showGate();
    this.restore();
  }
  ...
  async restore(){
    // لا نحفظ وضع الزائر بين الزيارات.
    localStorage.removeItem('zilzal_guest');
    if(!this.sb) return;
    try{
      const {data:{session}}=await this.sb.auth.getSession();
      if(session) await this.loadProfile({keepGate:true});
    }catch(_){}
  }
  ...
  showGate(tab='login'){
    ...
    if(btn) btn.click();
    setTimeout(()=>{
      const field=document.getElementById(tab==='signup'?'z-signup-name':'z-login-id');
      field?.focus?.();
    },60);
  }
  hideGate(){const gate=document.getElementById('zilzal-auth-gate');if(gate){gate.classList.add('hidden');gate.style.display='none';}}

  async enterGuest(){
    try{if(this.sb) await this.sb.auth.signOut();}catch(_){}
    this.guest=true;
    this.profile=null;
    this.hideGate();
    this.paint();
  ...
  async login(){
    try{
      ...
      const id=document.getElementById('z-login-id').value.trim(), password=document.getElementById('z-login-password').value;
      const x=await this.api('auth-login',{id,password});
      await this.sb.auth.setSession({access_token:x.access_token,refresh_token:x.refresh_token});
      const ok=await this.loadProfile();
      if(!ok) throw new Error('تم تسجيل الدخول لكن تعذر تحميل بيانات الحساب');
      this.hideGate();
      this.msg('');
  ...
  async loadProfile(opts={}){
    ...
    if(!user){this.profile=null;this.paint();if(!opts.keepGate)this.showGate();return false;}
    const {data,error}=await this.sb.from('profiles').select('*').eq('id',user.id).maybeSingle();
    if(error||!data){this.profile=null;this.guest=false;this.paint();if(!opts.keepGate)this.showGate();return false;}
    this.profile=data;
    this.guest=false;
    this.paint();
    this.startTimer();
  ...
  needAccount(text='هذه الميزة للحسابات المسجلة فقط.'){
    ...
    document.getElementById('go-auth-from-modal')?.addEventListener('click',()=>{this.showGate('login');});
  }
...
window.addEventListener('pageshow',(e)=>{if(e.persisted&&window.zilzalAccounts)window.zilzalAccounts.showGate('login');});
